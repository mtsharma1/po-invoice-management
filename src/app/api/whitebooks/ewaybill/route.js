...
import { generateSandboxEwayBill, getSandboxEwayBill } from '@/lib/whitebooksEwayBill';
...
async function handle(request, generate) {
  try {
    ...
    const result = generate
      ? await generateSandboxEwayBill(invoiceNo, payload.transport, payload.draft)
      : { ok: true, submission: await getSandboxEwayBill(invoiceNo) };
