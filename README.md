...
## Production standalone e-way bills
Set WHITEBOOKS_EWAYBILL_ENV=production and the WHITEBOOKS_EWAYBILL_PRODUCTION_* account values listed in .env.example. E-way bill client ID and secret are separate from e-invoice client credentials. Both standalone authentication and generation use https://api.whitebooks.in. The UI identifies the active environment and confirms a production submission.

Production requests and generated details are stored in webWhitebooksProductionEwayBill, keyed by InvoiceNo, with a unique DocumentKey to prevent concurrent duplicate generation. ResultJson contains EwbNo, EwbDt and EwbValidTill. Returning to the invoice reloads these details automatically. Sandbox results are never treated as production bills. Existing production e-way bills generated with an IRN are recognized from webWhitebooksProductionIrn. A pending/uncertain submission requires reconciliation, not regeneration. The table is created on first use and needs CREATE TABLE permission. Production standalone authentication must be verified with this account before live generation; combined IRN generation continues using the e-invoice production account.
